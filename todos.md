...
- [x] Add validation for export configurations
- [x] Implement proper handling of aggregations with the nullable aggregator field
...
- **Added LayoutValidator, export:validate command, and localizable messages; release wrap-up** (2026-09-02, planned via plan mode): New read-only `LayoutValidator` service (`validate`/`validateDraft`, never writes or lazy-syncs) checks models, columns, filters, sorts, definitions, and pivot configs for ~25 problem classes (errors break exports; warnings flag suspicious-but-functional config). Every problem carries `severity`/`code`/`source`/`params`/`message`; messages render via the `laravel-exports::validation` lang namespace (`lang/en/validation.php`, publishable with the `lang` tag) so consuming projects override wording or add locales - the studio UI can map `code`+`params` to its own strings. Creation-time integration: `ExportLayoutBuilder::validate()` spot-checks staged layouts and `save()` pre-validates, throwing all errors at once. New `export:validate {--layout=}` command prints per-layout problem tables with a CI-friendly exit code. Wrap-up: README rewritten around the builder/zero-setup flow with current features and the array-output extension point for custom formats; CLAUDE.md untracked and CLAUDE.md/CLAUDE.local.md/.claude/ gitignored; remaining todo checkboxes resolved (validation done; nullable aggregator verified long-complete; Excel shipped, PDF intentionally out of scope; UI handled by laravel-exports-studio). Suite: 162 passing (636 assertions), PHPStan level 5 clean.
- **Adversarial test round: 33 new tests, 3 production defects fixed** (2026-09-02): Two theorist agents proposed edge-case/regression tests with hand-computed expectations; two adversarial agents then mutant-tested every proposal (re-introducing each claimed regression to prove the assertion flips), killed vacuous or nondeterministic specs, corrected wrong expected values, and independently confirmed defects. Fixes shipped: (1) `or` filters now group with the preceding filter - `(A OR B) AND C` - instead of flat SQL precedence that let an or-pair disjoin a scoping filter; (2) filter application order is deterministic (`filters()` orders by ordered-uuid id; previously database index order silently decided or-placement); (3) `applyColumnFilters` coerces comma-string request values for `in`/`not_in`/`between` via a shared `coerceRequestValue()` (previously `'120,30'` matched nothing while `['120','30']` worked). New coverage: filter conflict suppression, or grouping, pivot `pivot_filter_param` + hidden-column total exclusion, required/comma column filters, whereHas existence without row duplication + `getExportCount`, override/format/default ordering, filtered expansion headers, sort priority interleaving, pagination meta/slicing, smart dotted `not_in` with all request-key variants, position clamping, streaming CSV/JSON/XLSX across chunk boundaries (header-once, valid JSON, sheet routing, empty workbook title, stream-path formula sanitization, BOM), full `ProcessExportJob` lifecycle (csv/json, unsupported format failure status, zero-row completion), SchemaSync suppression semantics (`verify` hash trust, `syncOnce`, stub upgrade via `describe`), `validateNestedPath` segment contracts, handler option opt-outs, and `--deep` nested discovery incl. the visited-pop and circular-path guards. Suite: 154 passing (600 assertions), PHPStan level 5 clean.
- **Lazy-synced catalog, model-class layouts, row-carried filters/sorts, and the fluent builder** (2026-09-01, branch `feat/lazy-catalog-builders` off `chore/audit-l13-simplify`, planned and approved via plan mode): New `SchemaSync` service (extracted from ImportModelsCommand's inline sync, which now delegates) with `ensureFresh`/`syncModel`/`syncOnce`/`syncPath`/`syncColumnPath`/`describe`. Config `laravel-exports.schema_sync` = `lazy` (default: sync on first reference, trust existing rows) | `verify` (re-sync on `export_models.schema_hash` drift) | `manual` (never write, throw with import-models hint). Layouts gained nullable `model` FQCN (export_model_id now nullable; model wins; `ExportLayout::resolveExportModel()`), `filter_definitions` and `sort_definitions` JSON built as unsaved models with REAL catalog FKs via `resolveRelationRow()` (lazy path classification: base column, relation, nested chain via syncPath, dotted attribute via syncColumnPath riding smart filters). `buildRelationPath` traversal and `validateAndCreateNestedPath` now sync-as-referenced through SchemaSync (ad-hoc `createNestedRelation` insert deleted; manual mode makes exports write-free). Migration `2026_09_01_000003`: de-dupes then adds UNIQUE(export_model_id, relation, is_column) for race-safe upserts, plus the new columns. `ExportLayoutBuilder` (final, fluent, transactional) builds catalog-backed layouts from a model class with zero setup; `SchemaSync::describe()` is the UI picklist endpoint. 15 new tests (SchemaSyncTest + regression: empty-catalog exports, sync modes, definition filters/sorts incl. required request filters and sort_column, builder round trip and rollback). Suite: 120 passing, PHPStan level 5 clean.
